...
### 1.2 가입 승인 + 데이터 격리 아키텍처 (Phase 5, 설계 완료·코드 적용 전)

```
[텔레그램 클라이언트]
   │ "/start" 전송                              │ 미니앱 오픈 (메뉴 버튼)
   ▼                                             ▼
POST /telegram/webhook                    프론트엔드가 fetch 헤더에
(api/webhook.py)                          X-Telegram-Init-Data 첨부
   │                                             │
   ├─ 이미 승인 → 안내                            ▼
   ├─ 대기 중 → 안내                      require_telegram_auth
   └─ 신규 → 대기 등록 +                  (api/telegram_auth.py)
      관리자에게 승인/거절 버튼 전송         initData HMAC 검증
             │                             → access_control.is_allowed()
             ▼                                    │
   관리자 [승인]/[거절] 클릭                        ├─ 미승인 → 403
   → callback_query 처리                          └─ 승인 → user_id 확보
   → access_control.add_allowed_user()                   │
                                                          ▼
                                            엔드포인트 로직 실행 시
                                            user_id를 generate_draft.py에
                                            전달 → DATA_DIR/projects/{user_id}/...
                                            로 격리 저장. /generate는
                                            api/rate_limit.py로 일일 5회 제한
```

- 봇과 API는 하나의 FastAPI 프로세스(단일 Railway 서비스)로 통합, 웹훅 방식 사용 (폴링 아님)
- 영구 저장소는 Railway Volume(`/data`, 환경변수 `RAILWAY_VOLUME_MOUNT_PATH`로 자동 주입)을
  `common.py`의 `DATA_DIR`이 참조
- 상세 설계(파일별 함수 시그니처, HMAC 검증 알고리즘, 승인 플로우 메시지 문구 등)는
  `docs/PHASE5_SIGNUP_APPROVAL.md` 참조 — 이 문서는 요약만 다룬다
...
```
safety-rag/
├── .env                                  ← Git 제외 (API 키, 봇 토큰 등)
├── .gitignore                            ← `data/` 추가 예정 (Phase 5, DATA_DIR 로컬 폴백 경로)
├── requirements.txt                      ← `requests` 추가 예정 (Phase 5, telegram_bot.py용)
├── Procfile                              ← Railway 배포용 (uvicorn 실행 명령)
├── common.py                             ← DATA_DIR 상수 추가 예정 (Phase 5)
├── build_knowledge_base.py
├── generate_draft.py                     ← CLI 진입점 + API가 재사용하는 핵심 함수,
│                                            user_id 인식 리팩터링 예정 (Phase 5)
├── test_search.py
...
│   ├── 산업안전보건관리비_가이드.txt
│   └── 표준작업계획서_가이드.txt
├── projects/                             ← 레거시 저장 위치. Phase 5부터는 신규 기록이
│                                            `{DATA_DIR}/projects/{user_id}/{현장명}.json`으로
│                                            이동, 기존 데이터는 마이그레이션 없이 그대로 유지
├── data/                                 ← Git 제외, 로컬 개발용 DATA_DIR 폴백 경로 (Phase 5
│                                            신규, Railway에서는 Volume 마운트 경로 `/data`가 대체)
├── api/                                  ← FastAPI 백엔드
│   ├── main.py                           ← 앱 진입점, CORS, 정적파일 마운트(`/app`), 헬스체크(`/`).
│   │                                        webhook_router 등록 + startup 시 set_webhook() 호출
│   │                                        추가 예정 (Phase 5)
│   ├── routes.py                         ← 엔드포인트 (generate_draft.py 함수 재사용).
│   │                                        Depends(require_telegram_auth) 적용 예정 (Phase 5)
│   ├── schemas.py                        ← Pydantic 요청/응답 스키마
│   ├── telegram_auth.py                  ← initData HMAC 검증 (Phase 5 신규 작성 예정 —
│   │                                        기존 인증 로직 자체가 없음, 검증 결과는
│   │                                        docs/PHASE5_SIGNUP_APPROVAL.md 3절 참조)
│   ├── telegram_bot.py                   ← (Phase 5 신규 예정) send_message 등 봇 API 호출 헬퍼
│   ├── access_control.py                 ← (Phase 5 신규 예정) 승인/대기 목록 파일 기반 관리
│   ├── rate_limit.py                     ← (Phase 5 신규 예정) 인당 일일 사용량 제한(KST 기준)
│   └── webhook.py                        ← (Phase 5 신규 예정) POST /telegram/webhook 엔드포인트
├── webapp/                               ← 텔레그램 미니앱 프론트엔드
│   └── index.html                        ← 단일 파일 SPA, Telegram Web App SDK 연동.
│                                            fetch 헤더에 X-Telegram-Init-Data 추가 예정 (Phase 5)
└── docs/
    ├── PRD.md / PLAN.md / SPEC.md
    └── PHASE5_SIGNUP_APPROVAL.md          ← Phase 5 전용 상세 설계·구현 계획 문서
```
...
**Phase 5 추가 예정**: `requests` (텔레그램 Bot API 호출용, `api/telegram_bot.py`에서 사용)
...
**Phase 5 추가 예정** (Railway Variables 탭, 상세는 `docs/PHASE5_SIGNUP_APPROVAL.md` 5절):
`ADMIN_TELEGRAM_USER_ID`(관리자 텔레그램 ID, 항상 승인 상태), `TELEGRAM_WEBHOOK_SECRET`(웹훅
요청 진위 검증용 임의 문자열), `PUBLIC_APP_URL`(웹훅 등록 시 사용할 배포 도메인). `DATA_DIR`은
Railway가 자동 주입하는 `RAILWAY_VOLUME_MOUNT_PATH`를 코드가 직접 참조하므로 별도 등록 불필요.
...
로컬 개발 시 `./data`, Railway에서는 자동 주입되는 `RAILWAY_VOLUME_MOUNT_PATH`(Volume 마운트
경로 `/data`)를 그대로 사용. `generate_draft.py`의 프로젝트 저장 경로와 Phase 5 신규 모듈
(`access_control.py`, `rate_limit.py`)의 저장 파일이 모두 이 경로를 기준으로 삼는다.
...
#### `generate_document_draft(document_type, project_info, project_name=None, risk_assessment_record=None) -> (draft, saved_record)`
1. 검색 쿼리 생성: `f"{document_type} 작성 관련 {project_info}"`
2. `search_similar_chunks(query, top_k=5)` 호출 (기본 모델 Voyage 사용), 결과를 콘솔에 디버그 출력
3. 이미 선택된 `risk_assessment_record`가 있으면(호출부가 사전에 `choose_risk_assessment()` 등으로
   선택) `linked_risk_context`로 프롬프트에 추가 — CLI/API 공용화를 위해 이 함수 내부에서는
   더 이상 대화형으로 회차를 고르지 않는다 (Phase 4에서 변경)
4. system prompt: 참고자료 기반 작성, 실제 위험성평가표 있으면 우선 반영, 위험성 점수는
   "(AI 제안값, 현장 확인 필수)" 라벨과 함께 반드시 채움, 그 외 날짜·서명 등은 추측 금지,
   결과 하단에 법적 고지 문구 강제 포함
5. Claude API 호출 (`model="claude-sonnet-4-6"`, `max_tokens=4000`)
6. `project_name` 있으면 결과를 `save_project_record()`로 저장, `(draft, saved_record)` 튜플 반환
   (API가 저장된 레코드의 `id`를 응답에 담기 위함)

**Phase 5 변경 예정**: `user_id` 파라미터 추가, 저장 경로가 `{DATA_DIR}/projects/{user_id}/{현장명}.json`으로
변경 (다중 사용자 데이터 격리). 상세는 `docs/PHASE5_SIGNUP_APPROVAL.md` 4절-② 참조.
...
- `app.mount("/app", StaticFiles(directory="webapp", html=True))`로 프론트엔드 정적 서빙
  (라우터를 먼저 `include_router`하고 그 뒤에 마운트해야 API 경로가 정적 파일 마운트에
  가로채이지 않음 — 실제 버그로 확인 후 순서 고정, 9번 "알려진 한계" 참조)
- **Phase 5 변경 예정**: `webhook_router` 등록, `@app.on_event("startup")`에서 `set_webhook()`
  자동 호출 (상세는 `docs/PHASE5_SIGNUP_APPROVAL.md` 4절-⑧)
...
- **Phase 5 변경 예정**: 4개 엔드포인트 모두 `Depends(require_telegram_auth)` 추가, 검증된
  `user_id`를 `generate_draft.py` 호출에 전달, `POST /generate`에 `rate_limit.check_and_increment`
  적용(초과 시 429). 현재는 인증이 전혀 없어 누구나 호출 가능한 상태 (9번 "알려진 한계" 참조)
...
### 6.7 Phase 5 신규 모듈 (설계 완료, 코드 적용 전)

아래 모듈들은 아직 파일 자체가 존재하지 않는다. 함수 시그니처·알고리즘 등 상세 스펙은
`docs/PHASE5_SIGNUP_APPROVAL.md` 4절에 있고, 여기서는 각 모듈의 책임만 요약한다.

| 모듈 | 책임 |
|---|---|
| `api/telegram_bot.py` | 텔레그램 Bot API 호출 헬퍼(`send_message`, `edit_message_text`, `answer_callback_query`, `approve_reject_keyboard`, `set_webhook`) |
| `api/access_control.py` | 승인/대기 목록 파일 기반 관리(`allowed_users.json`, `pending_requests.json`), `threading.Lock`으로 동시 쓰기 보호, `ADMIN_TELEGRAM_USER_ID`·`DAILY_LIMIT` 상수 정의 |
| `api/rate_limit.py` | 인당 일일 사용량 제한(`usage_log.json`), KST 고정 기준으로 자정 리셋 |
| `api/telegram_auth.py` | 텔레그램 Mini App initData의 HMAC-SHA256 서명 검증 + `access_control.is_allowed()` 연동 `Depends` 함수. **주의**: `PROJECT_HANDOFF.md`는 이 파일이 "환경변수 방식에서 파일 기반 방식으로 교체될 기존 파일"이라 설명했으나, 실제로는 파일 자체가 없어 처음부터 새로 작성해야 함이 착수 전 검증에서 확인됨 |
| `api/webhook.py` | `POST /telegram/webhook` — `/start` 가입 신청 처리, 승인/거절 인라인 버튼 콜백 처리, `X-Telegram-Bot-Api-Secret-Token` 헤더로 요청 진위 검증 |
...
| `api/main.py`에 `webapp/index.html` 정적 서빙 라우트가 없어 로컬에서 `/`, `/index.html` 등으로 직접 접근 시 404 | ✅ 해결 — `StaticFiles` 마운트 추가, 실기기 테스트까지 완료 (Phase 4) |
| 현재 API에 인증이 없어 현장명만 알면 누구나 해당 현장 기록을 조회·생성 가능 | 🟡 Phase 5에서 해결 예정 — 설계 완료, 코드 적용 전. `docs/PHASE5_SIGNUP_APPROVAL.md` 참조 |
| 프로젝트 데이터가 사용자별로 격리되지 않아 같은 현장명 사용 시 기록이 섞임 | 🟡 Phase 5에서 해결 예정 — `user_id` 기반 저장 경로 분리로 설계 완료, 코드 적용 전 |
