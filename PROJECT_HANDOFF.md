...
> 이 문서 하나로 Phase 4(텔레그램 미니앱화) + 현재 진행 중인 "가입 승인 플로우 + Railway Volume" 작업을 모두 다룹니다. (이전에 문서가 2개로 나뉘어 있었으나 이 파일로 통합, 기존 두 파일은 더 이상 관리하지 않아도 됩니다.)
...
| Phase 5 (진행 중) | 가입 승인 플로우 + Railway Volume | 🟡 설계 완료, 코드 적용 전 (5절) |
...
- **환경변수 추가 필요**: `ADMIN_TELEGRAM_USER_ID=(본인 텔레그램 ID)`, `TELEGRAM_WEBHOOK_SECRET=(임의의 긴 문자열)`, `PUBLIC_APP_URL=https://web-production-9d1bd.up.railway.app`
  (`DATA_DIR`은 자동 주입되는 `RAILWAY_VOLUME_MOUNT_PATH`를 코드에서 직접 참조하므로 별도로 추가할 필요 없음)
- **환경변수 제거**: 기존 `ALLOWED_TELEGRAM_USER_ID` 또는 `ALLOWED_TELEGRAM_USER_IDS` (파일 기반으로 대체되므로 불필요)
...
├── generate_draft.py             (문서 생성 핵심 로직 — CLI/API 공용, user_id 리팩터링 예정)
...
│   ├── 산업안전보건관리비_가이드.txt
│   └── 표준작업계획서_가이드.txt
├── projects/                     (Git 제외 — 현장별 생성 기록. 다음 작업에서
│                                    {DATA_DIR}/projects/{user_id}/{현장명}.json 구조로 변경 예정)
├── usage_log.json                (Git 제외 — 아직 미구현, 다음 작업에서 추가 예정)
├── api/
│   ├── __init__.py
│   ├── main.py                   (FastAPI 앱, CORS, 라우터 등록, 정적파일 마운트 /app)
│   ├── routes.py                 (4개 엔드포인트)
│   ├── schemas.py                (Pydantic 모델)
│   └── telegram_auth.py          (initData HMAC 검증 — 파일 기반 방식으로 교체 예정)
│   [다음 작업에서 추가 예정: telegram_bot.py, access_control.py, rate_limit.py, webhook.py]
├── webapp/
...
│   ├── PLAN.md
│   └── SPEC.md
└── (Git 훅) .git/hooks/pre-commit  (knowledge_base 수정 시 embeddings_voyage.json 동반 커밋 강제)
```
...
| 텔레그램 웹훅 | 미설정 (다음 작업에서 등록 예정) |

---

## 7. Claude Code에서 시작할 때 이렇게 말씀하시면 됩니다

> "이 핸드오프 문서 읽고, 5절(가입 승인 플로우 + Railway Volume)부터 이어서 코드 적용해줘"

라고 하시면서 이 파일을 프로젝트 루트에 두고 시작하시면, 5-2절의 순서(①~⑨)대로 실제 파일 생성/수정, 5-3절 환경변수 설정 안내, 5-4절 테스트까지 이어서 진행할 수 있습니다.
