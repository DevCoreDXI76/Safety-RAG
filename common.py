...
# 2026-07: 위험성평가표를 곱셈법(가능성×중대성=1~25 숫자, 구간 "1~4"/"5~9"/
# "10~25")에서 행렬법(빈도×강도 → A/B/C 등급, 위험성평가_실시규정.txt 3절)으로
# 전환했다. 전환 직후 가장 현실적인 회귀는 모델이 예전 학습 패턴대로 구식
# 숫자 구간표를 다시 출력하는 것 — 새 방식에는 이 라벨들이 나올 이유가 전혀
# 없으므로, "존재하면 곧 구식 표기로 돌아갔다는 뜻"인 가드레일로 뒤집었다
# (예전 체커는 반대로 "정확한 라벨이 사라졌는지"를 봤다 — 그 문제 자체가
# 이제 없다: 행렬법은 물결표 있는 구간을 쓰지 않는다).
DEPRECATED_RISK_SCORE_LABELS = ["1~4", "5~9", "10~25"]


def find_deprecated_risk_score_labels(draft):
    """
    _finalize_draft에서 document_type == "위험성평가표"일 때만 호출된다.
    구식 곱셈법 구간표("1~4 낮음" 등)가 draft에 남아있으면 반환한다 — 있으면
    행렬법(A/B/C) 대신 예전 숫자 점수 방식으로 회귀했다는 뜻이다.
    """
    return [label for label in DEPRECATED_RISK_SCORE_LABELS if label in draft]
