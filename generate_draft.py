...
from common import (
    ...
    find_deprecated_risk_score_labels,
...
def choose_document_type():
    ...
    while True:
        ...
        if choice in DOCUMENT_TYPES:
            if DOCUMENT_TYPES[choice].startswith("기타"):
...
def _build_generation_prompt(document_type, project_info, project_name=None, risk_assessment_record=None, work_type=None):
    ...
    system_prompt = (
        ...
        "위험성평가표를 작성하는 경우, 각 위험요인마다 빈도(가능성)·강도(중대성)·위험등급을 "
        "반드시 제안해. 빈도·강도는 1(낮음)/2(보통)/3(높음) 중 하나의 숫자로, 위험등급은 그 "
        "둘을 참고자료의 3×3 행렬(빈도×강도 → A/B/C)에 그대로 대입해 A(매우위험)/B(위험)/"
        "C(주의관리요) 중 하나로 산정해 — 빈도와 강도를 곱한 숫자 점수를 만들지 마, 반드시 "
        "행렬 조회 결과인 A/B/C 등급으로 적어. 이 기준은 참고자료(위험성평가 실시규정 등)에 "
        "명시된 행렬을 그대로 따르고, 참고자료에 다른 기준이 명시되어 있다면 그 기준을 "
        "우선해. 같은 문서 안의 위험요인들은 실제 위험의 성격(예: 질식·추락처럼 중대재해로 "
        "이어지기 쉬운 위험과, 경상 수준에 그치는 위험)에 따라 빈도·강도가 서로 다르게 "
        "나오도록 항목별로 신중히 판단하고, 근거 없이 여러 항목에 동일한 값을 기계적으로 "
        "반복하지 마. 위험성 감소대책을 적용했다고 가정한 '개선후 위험등급'도 같은 행렬로 "
        "재산정해서 적어 — 통상 원래 등급보다 같거나 낮아야 해. 단, 빈도·강도·위험등급· "
        "개선후 위험등급은 실제 현장 데이터가 아니라 참고자료 기준에 따른 'AI 제안값'이므로, "
        "값을 적을 때마다 반드시 그 옆에 '(AI 제안값, 현장 확인 필수)'라고 표시해. 절대 "
        "빈칸으로 남기지 마.\n\n"
        "위험성평가표가 아닌 다른 문서(표준 작업계획서 등)를 작성하는 경우에는, 참고자료에 "
        "위험성 추정 기준(행렬, 등급 판정표)이 실제로 포함되어 있지 않은 이상 위험등급이나 "
        "행렬표(예: '빈도×강도 → A/B/C' 같은 형태)를 임의로 만들어 넣지 마. "
        "위험성평가와의 연계를 언급할 필요가 있으면 '해당 작업에 대해 별도로 작성된 "
        "위험성평가표를 참조'처럼 결과 반영 원칙만 서술하고, 구체적인 등급·행렬 수치는 "
        "적지 마.\n\n"
        ...
        "참고 자료에 없는 내용 중 빈도·강도·위험등급을 제외한 나머지(날짜, 인명, 서명 등 실제 "
        "현장 정보)는 추측해서 만들어내지 말고 빈칸으로 남겨. 실제 서류처럼 항목과 형식을 갖춰서 "
        "작성해.\n\n"
        "숫자 구간을 표기할 때는 물결표(~)를 절대 생략하지 마. 예를 들어 '1~3', '50~250'처럼 "
        "반드시 물결표를 넣어서 쓰고, '13', '50250'처럼 물결표 없이 숫자를 붙여 쓰지 마.\n\n"
        ...
        "작업 단계나 항목별로 담당자·수행 인원을 표에 적을 때, 특정 개인이 아니라 현장에 참여하는 "
        "모든 인원이 함께 수행·확인해야 하는 경우에는 '전원'이라는 표현 대신 '전체 인원'이라고 명확히 써. "
        "'전원'은 전기 관련 문서에서 전원(電源, 전기 공급)이라는 뜻으로도 쓰여서, 특히 전기작업 "
        "내용이 함께 있는 문서에서는 혼동을 피하기 위해 이 구분을 반드시 지켜.\n\n"
...
def _finalize_draft(draft, context, linked_risk_context, document_type, project_info, project_name, user_id, work_type=None):
    ...
    # 구식 곱셈법 구간표(1~4/5~9/10~25)가 위험성평가표에 남아있는지는
    # 위험성평가표에서만 의미가 있다. 다른 문서(표준 작업계획서 등)는
    # system_prompt 지시에 따라 "위험성평가표 참조"처럼 단어만 언급하고
    # 구체 수치는 적지 않는 게 정상 동작이라, document_type으로 걸러야
    # 오탐을 피한다.
    deprecated_labels = (
        find_deprecated_risk_score_labels(draft) if document_type == "위험성평가표" else []
    )
    if deprecated_labels:
        print(f"[WARN] 구식 위험성 점수 구간표 사용(행렬법 회귀 의심): {deprecated_labels}")
        range_warning = (
            "\n\n> ⚠ **자동 검증 알림**: 이 초안에 구식 위험성 점수 구간표 표기가 "
            f"남아있습니다 ({', '.join(deprecated_labels)}). 현재 방식은 빈도×강도 "
            "행렬로 위험등급(A/B/C)을 산정하는 것이니, 숫자 구간표가 섞여 있다면 "
            "반드시 재확인하세요."
        )
        draft += range_warning
